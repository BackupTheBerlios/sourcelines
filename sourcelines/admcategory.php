...
if ($action == "db_add") {						// DB ADD
	$query = "INSERT tblcategory SET category_name='$name', category_parent='$cate_id'";
	if (!$db->query($query)) mysql_die($db);
} 
else if ($action == "db_delete") {				// DB DELETE
	$query = "DELETE FROM tblcategory WHERE category_id='$cate_id'";
	if (!$db->query($query)) mysql_die($db);
}
else if ($action == "db_change") {				// DB CHANGE
	$query = "UPDATE tblcategory SET category_name='$name' WHERE category_id=$cate_id";
	if (!$db->query($query)) mysql_die($db);
}

if ($action == "add") {							// ADD
	$cate_id   = rawurldecode($cate_id);
	/*-----------------------------------------------------------------------*/
	$bx->box_begin();
	$bx->box_title($t->translate("Add Category"));
	$bx->box_body_begin();
	/*-----------------------------------------------------------------------*/
	...
	/*-----------------------------------------------------------------------*/
	$bx->box_begin();
	$bx->box_title($t->translate("Change Category"));
	$bx->box_body_begin();
	/*-----------------------------------------------------------------------*/
	echo "<form action='".$sess->url(basename($PHP_SELF))."' method='POST'>";
	$tbw->table_begin();
