...
#
# Administrate types of components
#
...
if($action=="db_change") {						// DB CHANGE
	$query = "UPDATE tblcomponenttype SET componenttype_name='$name' WHERE componenttype_id=$id";
	if (!$db->query($query)) mysql_die($db);
}
else if($action=="db_delete") {					// DB DELETE
	$query = "DELETE FROM tblcomponenttype WHERE componenttype_id=$id ";
	if (!$db->query($query)) mysql_die($db);
}
else if($action=="db_add") {					// DB_ADD
	$query = "INSERT tblcomponenttype(componenttype_name) VALUES('$name') ";
	if (!$db->query($query)) mysql_die($db);
}
...
if($action=="change") {							// CHANGE
	...
	/*-----------------------------------------------------------------------*/
	$bx->box_begin();
	$bx->box_title($t->translate("Change type of component"));
	$bx->box_body_begin();
	/*-----------------------------------------------------------------------*/
	...
	/*----------*/
	echo "<b>".$t->translate("Type of Component").":</b>";
	...
	echo "</form>";
	/*==========*/
	echo "<form action='".$sess->url("admcomptype.php")."' method='POST'>";
	$tbw->table_body_column_begin();
	...
	/*-----------------------------------------------------------------------*/
	$bx->box_begin();
	$bx->box_title($t->translate("Delete type of component"));
	$bx->box_body_begin();
	/*-----------------------------------------------------------------------*/
	echo "<form action='".$sess->url(basename($PHP_SELF))."' method='POST'>";
	$tbw->table_begin();
	...
	$bx->box_begin();
	$bx->box_title($t->translate("Add type of component"));
	$bx->box_body_begin();
	/*-----------------------------------------------------------------------*/
	echo "<form action='".$sess->url(basename($PHP_SELF))."' method='POST'>";
	$tbw->table_begin();
	...
	/*------------------------------------------------------- BOX tbllicense */
	$bx->box_begin();
	$bx->box_title($t->translate("Type of Component Administration"));
	$bx->box_body_begin();
	/*------------------------------------------------------ gesamte Tabelle */
	...
	while($db->next_record()) {
		...
		/*----------*/
		echo $db->f("componenttype_name");
		/*----------*/				
		$tby->table_body_column_next();
		echo '<b><a href="'.$sess->url(basename($PHP_SELF)).$sess->add_query(array("action"=>"change", "id"=>$db->f("componenttype_id"), "name"=>$db->f("componenttype_name"))).'">'.$t->translate("Change").'</a></b>';
		/*----------*/
		$tby->table_body_column_next();
		echo '<b><a href="'.$sess->url(basename($PHP_SELF)).$sess->add_query(array("action"=>"delete", "id"=>$db->f("componenttype_id"), "name"=>$db->f("componenttype_name"))).'">'.$t->translate("Delete").'</a></b>';
		/*----------*/
	...
	$tbw->table_row_begin();
	$tbw->table_body_column_begin();
	echo "<b><a href='".$sess->url(basename($PHP_SELF)).$sess->add_query(array("action"=>"add"))."'>".$t->translate("Add")."</a></b> ".$t->translate("new type of component");
	$tbw->table_body_column_end();
	$tbw->table_row_end();
	$tbw->table_end();
	/*-----------------------------------------------------------------------*/
