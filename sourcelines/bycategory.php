...
#
# Shows solutions selected by category.
#
...
if ($db->num_rows() > 0) {
	$sort = $t->translate("sorted by").": "
		."<a href=\"".$sess->url(basename($PHP_SELF)).$sess->add_query(array("by" => "Date", "cate_id" => $cate_id))."\">".$t->translate("Date")."</a>"
		." | <a href=\"".$sess->url(basename($PHP_SELF)).$sess->add_query(array("by" => "Name", "cate_id" => $cate_id))."\">".$t->translate("Name")."</a>"
		." | <a href=\"".$sess->url(basename($PHP_SELF)).$sess->add_query(array("by" => "Contact", "cate_id" => $cate_id))."\">".$t->translate("Contact")."</a>"
		." | <a href=\"".$sess->url(basename($PHP_SELF)).$sess->add_query(array("by" => "Supplier", "cate_id" => $cate_id))."\">".$t->translate("Supplier")."</a>\n";

	$bs->box_strip($sort);

	if ($cnt > 0) {
		$nav = "&lt;&nbsp;<a href=\"".$sess->url(basename($PHP_SELF)).$sess->add_query(array("cnt" => $next_cnt, "by" => $by, "cate_id" => $cate_id))."\">".$t->translate("previous")." $config_show_solsperpage&nbsp;".$t->translate("Solutions")."</a>";
	} else {
		$nav = "&lt;&nbsp;".$t->translate("previous")." $config_show_solsperpage&nbsp;".$t->translate("Solutions");
}

	$nav .= " | <a href=\"".$sess->url(basename($PHP_SELF)).$sess->add_query(array("by" => $by, "cate_id" => $cate_id))."\">".$t->translate("Top")."</a>";

	if ($db->num_rows() >=  $config_show_solsperpage) {
		$nav .= " | <a href=\"".$sess->url(basename($PHP_SELF)).$sess->add_query(array("cnt" => $prev_cnt, "by" => $by, "cate_id" => $cate_id))."\">".$t->translate("next")." $config_show_solsperpage ".$t->translate("Solutions")."</a>&nbsp;&gt;";
	} else {
