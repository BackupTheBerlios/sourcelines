...
if (!$ml_list) {
  ...
  while (is_array($HTTP_POST_VARS) && list($key, $val) = each($HTTP_POST_VARS)) {
    switch ($key) {
	  ...
	  debug("mail($ml_newsreqaddr,\"subscribe $password\",$message,\"From: $email_usr\nReply-To: $email_usr\nX-Mailer: PHP\")");
	  ...
	  debug("mail($ml_weeklynewsreqaddr,\"subscribe $password\",$message,\"From: $email_usr\nReply-To: $email_usr\nX-Mailer: PHP\")");
	  $msg = $t->translate("Congratulations")."! "
...
<tr valign=middle align=left>
<td></td><td>
<input type=radio name="period" value="daily" checked>&nbsp;<?php echo $t->translate("daily");?><br>
