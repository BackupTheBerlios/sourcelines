...
#
# Search solutions by their name.
#
...
// When there's a search for a blank line, we look for "xxxxxxxx"
  ...
  if ($db->num_rows() > 0) {
    ...
    ."<a href=\"".$sess->url(basename($PHP_SELF)).$sess->add_query(array("search" => $search, "by" => "Date"))."\">".$t->translate("Date")."</a>"
    ." | <a href=\"".$sess->url(basename($PHP_SELF)).$sess->add_query(array("search" => $search, "by" => "Name"))."\">".$t->translate("Name")."</a>"
    ." | <a href=\"".$sess->url(basename($PHP_SELF)).$sess->add_query(array("search" => $search, "by" => "Contact"))."\">".$t->translate("Contact")."</a>"
    ." | <a href=\"".$sess->url(basename($PHP_SELF)).$sess->add_query(array("search" => $search, "by" => "Supplier"))."\">".$t->translate("Supplier")."</a>\n";
