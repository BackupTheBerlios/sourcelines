...
#
# Shows solutions selected by branch.
#
...
$columns = "*";
$where = "tblsolutions.username = auth_user.username AND tblsolutions.branch_id = '$bran_id' AND tblsolutions.solutions_name != 'no_name'";
...
if ($db->num_rows() > 0) {
	$sort = $t->translate("sorted by").": "
		."<a href=\"".$sess->url(basename($PHP_SELF)).$sess->add_query(array("by" => "Date", "bran_id" => $bran_id))."\">".$t->translate("Date")."</a>"
		." | <a href=\"".$sess->url(basename($PHP_SELF)).$sess->add_query(array("by" => "Name", "bran_id" => $bran_id))."\">".$t->translate("Name")."</a>"
		." | <a href=\"".$sess->url(basename($PHP_SELF)).$sess->add_query(array("by" => "Contact", "bran_id" => $bran_id))."\">".$t->translate("Contact")."</a>"
		." | <a href=\"".$sess->url(basename($PHP_SELF)).$sess->add_query(array("by" => "Supplier", "bran_id" => $bran_id))."\">".$t->translate("Supplier")."</a>\n";

	$bs->box_strip($sort);

	if ($cnt > 0) {
		$nav = "&lt;&nbsp;<a href=\"".$sess->url(basename($PHP_SELF)).$sess->add_query(array("cnt" => $next_cnt, "by" => $by, "bran_id" => $bran_id))."\">".$t->translate("previous")." $config_show_solsperpage&nbsp;".$t->translate("Solutions")."</a>";
	} else {
		$nav = "&lt;&nbsp;".$t->translate("previous")." $config_show_solsperpage&nbsp;".$t->translate("Solutions");
	}

	$nav .= " | <a href=\"".$sess->url(basename($PHP_SELF)).$sess->add_query(array("by" => $by, "bran_id" => $bran_id))."\">".$t->translate("Top")."</a>";

	if ($db->num_rows() >=  $config_show_solsperpage) {
		$nav .= " | <a href=\"".$sess->url(basename($PHP_SELF)).$sess->add_query(array("cnt" => $prev_cnt, "by" => $by, "bran_id" => $bran_id))."\">".$t->translate("next")." $config_show_solsperpage ".$t->translate("Solutions")."</a>&nbsp;&gt;";
	} else {
